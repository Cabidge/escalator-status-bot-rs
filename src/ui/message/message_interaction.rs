...
pub trait ToInteractionHandle {
    fn to_handle<'a, const EPHEMERAL: bool>(
        &'a self,
        http: &'a Http,
    ) -> InteractionHandle<'a, EPHEMERAL>;
}
...
impl<'a, const EPHEMERAL: bool> MessageHandle for InteractionHandle<'a, EPHEMERAL> {
    async fn show(&mut self, view: View) -> Result<(), serenity::Error> {
        if self.sent {
            self.interaction
                .edit_original_interaction_response(self.http, |res| {
            ...
            self.interaction
                .create_interaction_response(self.http, |res| {
    ...
    async fn get_message(&self) -> Option<serenity::Message> {
        self.interaction
            .get_interaction_response(self.http)
            .await
            .ok()
    }
}
